...
namespace gazebo {
    ...
    void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
      ...
      rosPointPub = rosNode->advertise<sensor_msgs::PointCloud2>("/livox/lidar", 5);
    ...
    void LivoxPointsPlugin::OnNewLaserScans() {
      if (rayShape) {
        ...
        pcl::PointCloud<pcl::PointXYZI> pointcloud_xyzi;
        pointcloud_xyzi.resize(points_pair.size());
        ...
        for (auto &pair: points_pair) {
          ...
          if (verticle_index < verticalRayCount && horizon_index < rayCount) {
            ...
            pointcloud_xyzi[point_count] = pt;
            ++point_count;
          }
        }

        sensor_msgs::PointCloud2 cloud2_msg;
        pcl::toROSMsg(pointcloud_xyzi, cloud2_msg);
        cloud2_msg.header.stamp = ros::Time::now();
        cloud2_msg.header.frame_id = "livox_frame";
        rosPointPub.publish(cloud2_msg);
        ros::spinOnce();
